...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
        ...
        <a href="{{ route('venues.create') }}" class="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-xs shadow-sm transition-colors">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @foreach($venues as $venue)
            <div class="clean-card rounded-xl overflow-hidden flex flex-col justify-between group">
                <div>
                    ...
                    <div class="p-5 space-y-3 text-xs">
                        <div>
                            <h3 class="text-base font-bold text-slate-900 leading-tight">{{ $venue['name'] }}</h3>
                        ...
                        <div class="bg-slate-50 p-3 rounded-lg border border-slate-200 space-y-1.5">
                            <div class="flex justify-between items-center text-slate-500">
                                ...
                                <strong class="text-slate-800">{{ $venue['operating_hours'] }}</strong>
                            </div>
                            <div class="flex justify-between items-center text-slate-500">
                                <span>Jumlah Court:</span>
                                <strong class="text-emerald-700">{{ count($venue['courts']) }} Lapangan</strong>
                            </div>
                        </div>
                        ...
                        <div class="space-y-1">
                            ...
                            <div class="flex flex-wrap gap-1">
                                @foreach(array_slice($venue['facilities'], 0, 3) as $fac)
                                    <span class="px-2 py-0.5 rounded bg-slate-100 text-[10px] text-slate-600 border border-slate-200">
                                        {{ $fac }}
                                    </span>
                                @endforeach
                                @if(count($venue['facilities']) > 3)
                                    <span class="px-2 py-0.5 rounded bg-slate-100 text-[10px] text-slate-400 border border-slate-200">
                ...
                <div class="p-5 pt-0">
                    <a href="{{ route('venues.show', $venue['id']) }}" class="block text-center py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-800 font-semibold text-xs transition-colors">
                        Lihat Court & Jadwal
                    </a>
                </div>
            </div>
