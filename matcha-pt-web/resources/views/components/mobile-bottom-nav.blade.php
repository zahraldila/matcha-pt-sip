...
<nav class="md:hidden fixed bottom-0 inset-x-0 z-50 bg-white/95 backdrop-blur-2xl border-t border-slate-200/80 shadow-[0_-4px_24px_rgba(0,0,0,0.08)] px-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-1.5 transition-all">
    <div class="max-w-md mx-auto grid grid-cols-5 items-center">
        ...
        @if (Auth::check() && Auth::user()->isAdmin())
            {{-- ======================================================== --}}
            {{-- ADMINISTRATOR MODE: 5 Flat Action Tabs (No Floating Button) --}}
            {{-- ======================================================== --}}
            ...
            <!-- 4. Komunitas -->
            @php
                $isCommunities = request()->routeIs('communities.*');
            @endphp
            ...
            @endphp
            <a href="{{ route('admin.users') }}" class="flex flex-col items-center justify-center py-1 group transition-all">
                <div class="w-8 h-8 rounded-xl flex items-center justify-center transition-all {{ $isUsers ? 'bg-[#EBF8D8] text-[#063B00] shadow-2xs' : 'text-slate-500 group-hover:text-slate-800' }}">
                    <i class="fa-solid fa-users-gear text-sm"></i>
                </div>
                <span class="text-[10px] font-bold mt-0.5 tracking-tight {{ $isUsers ? 'text-[#063B00]' : 'text-slate-500 group-hover:text-slate-800' }}">
                    Pengguna
                </span>
            </a>
        ...
        @else
            {{-- ======================================================== --}}
            {{-- REGULAR / HOST / VENUE OWNER / GUEST MODE (Floating FAB) --}}
            {{-- ======================================================== --}}
            ...
            <!-- 3. Central Floating Action Button (Host Game / Add Venue / Register Community) -->
